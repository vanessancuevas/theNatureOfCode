...
class Jellyfish {
  ...
  update() {
    ...
    let d = this.pos.mag();
    let bounds = min(width, height) * 0.35;
    let maxBounds = min(width, height) * 0.45;
...
function draw() {
  ...
  let camX = sin(frameCount * 0.0005) * 100;
  let camZ = cos(frameCount * 0.0005) * 100;
  ...
  for (let p of particles) {
    p.y -= 0.8;
    p.x += sin(p.y * 0.01 + frameCount * 0.01) * 0.5;
    if (p.y < -height) {
      p.y = height;
