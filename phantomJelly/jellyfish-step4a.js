// ── P3 palette — pure channels, no grey mixing ─────────────────────────────
const C = {
  cyanR:0,    cyanG:230,  cyanB:255,   // bell nodes
  blueR:60,   blueG:100,  blueB:255,   // grid lines + tentacle fiber
  magR:255,   magG:20,    magB:210,    // rhopalia + memory
  gfnnR:255,  gfnnG:255,  gfnnB:255,   // GFNN sweep — pure white flash
  plankR:0,   plankG:180, plankB:255,  // background particles
};
...
new p5(function(sk) {
  ...
  // wx/wy: physics offset from canvas centre (local WEBGL space).
  // bobY (animation layer) is added on top at render time.
  let jelly = { wx: 0, wy: 0, vx: 0, vy: 0 };

  const JELLY_DRAG      = 0.92;   // velocity multiplier per frame (drag)
  const JELLY_SPEED_CAP = 4.0;    // maximum world-units / frame
  const JELLY_SPRING_K  = 0.012;  // spring stiffness toward tank safe-zone edge
  const JELLY_SAFE_FX   = 0.32;   // spring-free zone: fraction of half-width
  const JELLY_SAFE_FY   = 0.32;   // spring-free zone: fraction of half-height

  // ── stepJellyPhysics ───────────────────────────────────────────────────
  // Called once per draw(). Applies spring, drag, speed-cap, integration.
  // Future steps inject impulse forces via jelly.vx / jelly.vy.
  function stepJellyPhysics() {
    ...
    if (spd > JELLY_SPEED_CAP) {
      let inv = JELLY_SPEED_CAP / spd;
      jelly.vx *= inv;
      jelly.vy *= inv;
    }
  ...
  // ── Input ──────────────────────────────────────────────────────────────
  function handleTouch(mx,my,strength){
    document.getElementById('hint').style.opacity='0';
    let uv=screenToUV(mx,my);
    injectCharge(uv.u,uv.v,strength);
  }
  sk.mousePressed   =function(){handleTouch(sk.mouseX,sk.mouseY,0.9);};
  sk.mouseDragged   =function(){handleTouch(sk.mouseX,sk.mouseY,0.3);};
  sk.touchStarted   =function(){for(let t of sk.touches)handleTouch(t.x,t.y,0.9);return false;};
  sk.touchMoved     =function(){for(let t of sk.touches)handleTouch(t.x,t.y,0.25);return false;};
