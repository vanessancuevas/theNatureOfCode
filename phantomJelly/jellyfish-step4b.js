...
new p5(function(sk) {
  ...
  // ── Flee burst ─────────────────────────────────────────────────────────
  // On tap the effective speed cap is raised to FLEE_IMPULSE, then decays
  // back to JELLY_SPEED_CAP each frame — smooth burst, no hard cutoff.
  ...
  function nearestRhopaliuIdx(mx, my) {
    let cx = sk.width  * 0.5;
    let cy = sk.height * 0.5;
    let tx = mx - cx - jelly.wx;
    let ty = my - cy - (jelly.wy - 50);
    let touchU = (Math.atan2(ty, tx) / (2 * Math.PI) + 1) % 1;
